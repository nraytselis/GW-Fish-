...
ggplot(data=factorial_design, aes(x=Fish_abundance,y=Period,z=L3F_result)) + geom_contour_filled() + theme_classic() + theme(axis.text = element_text(size = 18)) + theme(axis.title = element_text(size = 25)) +
  theme(plot.margin = margin(1, 2, 1, 1, "cm")) + xlab("Fish Abundance")



####Pond sim over a fish density gradient### 
parameters = joint_fitB_best[[1]]
parameters["latent_stages"] = 60
parameters["latent_rate"] = 4.3
...
L3s = ggplot(data=data, aes(x = fish_vec, y = L3F_results)) + geom_line(linewidth=1) + theme_classic() + xlab("Fish Density (per L)") + ylab("L3 Parasites in Fish (per L)") +
...
fishchains_coda = convert.to.coda(fishchains)
library("stableGR")
stable.GR(joint_fit_chains) #476.1845 
#n.eff(joint_fit_chains)
#use parameters from best fit model 
